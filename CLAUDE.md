...
6. フロントエンド・バックエンドの挙動に関わる変更をした場合は `cd e2e && npm test` を実行し、E2Eテストが全て成功することを確認する（E2E側はバックエンド3100・フロントエンド5273という専用ポート（`e2e/ports.ts`）で動くため、`scripts/dev-up.sh` 等の開発サーバー（3000/5173）を止める必要はない）
