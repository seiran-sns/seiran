...
- **検索**: ローカル DB 全文検索（`idx_posts_body_bigm`、`idx_actors_username_bigm`／`idx_actors_display_name_bigm`／`idx_actors_acct_bigm`、pg_bigm）と AppView 検索の結果をマージする。pg_bigm は `LIKE` 演算子のみ最適化対象で `ILIKE` には対応しないため、検索 SQL は `LOWER(col) LIKE LOWER(pattern)` 形式に統一し、インデックスも `LOWER()` 適用後の式に張っている（#97）。セッション管理の詳細は `docs/architecture.md` の検索セッション節を参照。
