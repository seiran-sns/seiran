...
- `claimed_ap_object_id`/`claimed_at_uri`（nullable）: 他seiranサーバー間の投稿マージ（相互申告方式、`docs/protocols.md` 5節・#237）用。ATP起源で先着した行は`claimed_ap_object_id`に、AP起源で先着した行は`claimed_at_uri`に、`seiranPost.counterpartPostId`として自己申告された相手プロトコル側の真正IDを保持する。後から届いた実IDがこの自己申告と相互に一致した場合にのみ1行へマージし、成立時は`claimed_*`をクリアする。
