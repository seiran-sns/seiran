...
- **ATP（Bluesky）連携（投稿取り込み）**: 同じ Jetstream 接続で `app.bsky.feed.post` の create も検知する。DID が既知のローカル `actors`（`at_did` 一致）であれば、Jetstream イベントに同梱される `record.text`/`record.createdAt` をそのまま `posts` へ保存する。Jetstream はほぼリアルタイムでレコード本体を配信するため、旧実装にあった「AppViewへの再取得＋インデックス遅延リトライ（2s/5s/10s）」は不要になった。
