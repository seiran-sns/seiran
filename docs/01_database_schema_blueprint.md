...
- **既知の制限（スコープ外）**: `pinned_posts` 経由のピン留め一覧には可視性ガードが無い（将来課題）。
...
- **既知の制限（スコープ外）**: Fedi から受信した `Announce`（`handle_announce`,
  `inbound_activity_process.rs`）は可視性を判定せず常に `public` として保存する（リモートの
  可視性ポリシーは送信元サーバーが統制している前提のため今回未対応）。
