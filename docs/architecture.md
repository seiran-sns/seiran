...
- `components/right/` — 右ペインのタブ内容（`NotificationsPanel`、`TrendsSearchPanel`、`FollowListPanel`、`AuthorPanel`（ポスト詳細画面の「投稿者」タブ。投稿主のプロフィール概要と固定ポストを表示、`docs/ui_spec.md` 2.3参照）、`ReplyThreadPanel`（ポスト詳細画面の「返信」タブ。`GET /api/notes/:id/replies`のフラット配列から`replyId`/`quoteId`を辿ってツリーを再構築、`docs/ui_spec.md` 2.3参照））
