...
- **引用**: 元ポストが Fedi リモートのみの場合、Bsky側は投稿者名・本文・先頭画像（なければアバター）を持つ `app.bsky.embed.external` URLカード（at_uri/cidがあれば `app.bsky.embed.record`）。AP側は `ap_object_id` があればMisskey互換のquote URLに、無ければ bsky.app URL に変換する。ローカル・Fedi・Bskyのどの投稿も、Fedi/Bskyの両配送先を個別選択して引用できる。
