...
* **投稿作成UIでの可視性選択とガード（2026-07-17）**: `PostComposer` に可視性選択（👥パブリック/🤫ひかえめ/🔒️プライベート）を追加。Bsky（AT Protocol）はプロトコル上 public 投稿のみサポートするため、Bsky配送ON中に非publicへ変更しようとする、または非public中にBsky配送をONにしようとすると、フロントは軽量な自作ポップオーバー（`.popover`、汎用Tooltipコンポーネントは無いためこの機能に閉じて実装）でガイドし変更をブロックする。投稿APIはMisskey互換のためフロントを経由しない外部クライアントにはエラーを返せず、サーバー側（`create_regular_post`）で非public + Bsky配送要求を検知すると黙ってFediのみ配送に読み替える（`delivery.rs` の `deliver_regular_post` でも二重に防御）。
* **可視性による閲覧制御（2026-07-17）**: `followers_only`/`direct` は投稿者本人または accepted フォロワーのみ閲覧可、`unlisted` はホーム/ローカルタイムラインから除外する（詳細な判定ロジック・SQLガード式はDoc1 §1.3「可視性による閲覧制御」参照）。HTTP公開エンドポイントは可視性チェック付きの `PostRepository::find_by_id_for_viewer` を使い、ジョブ処理等の内部整合性チェックのみ従来の `find_by_id`（チェック無し）を使う。
