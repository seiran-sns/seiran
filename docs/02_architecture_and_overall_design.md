...
### 1.8 Note 詳細エンドポイント（AP エントリーポイント）

`/notes/:id` と `/post/:id` はパスを分離しており、コンテンツネゴシエーションは行わない。

| パス | 用途 | 転送先 |
|---|---|---|
| `/notes/:id` | ActivityPub（Fediverse サーバー向け）専用。連合先に公開する Note の正規 URL（`id` / `url`）としても使うため変更不可 | nginx が常に api（backend）へ転送 |
| `/post/:id` | ブラウザ向け SPA のポスト詳細画面 | nginx の catch-all（`location /`）で frontend へ転送 |

以前は `Accept` ヘッダーを見て転送先を切り替える nginx `map` ディレクティブを使っていたが、
ブラウザ向けパスを `/post/:id` に分離したことで不要になった（nginx はネイティブ環境では
`resolver`（Docker 内蔵 DNS 依存）が使えず動的アップストリームの解決ができないため、
静的な `proxy_pass` だけで完結する構成にした）。

#### ブラウザフロー

1. ブラウザが `/post/:id` を要求 → nginx が frontend へ転送
2. React Router が `/post/:id` にマッチし `NoteDetail` コンポーネントをレンダリング
3. `NoteDetail` が `GET /api/notes/:id` を呼んで投稿データを取得・表示

#### ActivityPub フロー

1. Fediverse サーバーが `GET /notes/:id` を要求
2. nginx が api へ転送
...
* **表示**: フロント `NoteCard` は `renote` があればヘッダ「（表示名）が（日時）にリポスト」＋元ポスト本体を描画する。ヘッダの日時は**リポスト自身**の詳細（`/post/{リポストID}`）へリンクし、リポスト直後の投稿を前後コンテキストで辿れるようにする。
* **詳細画面**: `/post/{リポストID}` を開いた場合、フォーカス投稿は元ポストの内容を本体に、上部にリポスト情報を添えて表示する。前後コンテキストは**リポスト実行者**の投稿列を対象とする。
