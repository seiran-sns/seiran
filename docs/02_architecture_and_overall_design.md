...
* **配送先・可視性バッジ（2026-07-16）**: `NoteCard` のヘッダ（アイコン名の `.acct` 行、既存の `protocolBadge` バッジの隣）に、ローカル投稿の配送先（🌐=Fedi配送あり、🦋=Bsky配送あり。`lib/format.ts` の `deliveryBadges`）と、可視性（🔒️=プライベート、🤫=ひかえめ。同 `visibilityBadge`）を表示する。いずれも既存の `.protoBadge` CSSクラスを流用した表示専用バッジ。配送先はDB永続化した `posts.deliver_fedi`/`deliver_bsky`（投稿作成時の選択をそのまま保存）、可視性はローカル投稿は投稿作成時の選択、Fedi受信ポストは `to`/`cc` から判定した `posts.visibility` に基づく（Doc1 §1.3「配送先・可視性アイコン表示」参照）。
* **投稿作成UIでの可視性選択とガード（2026-07-17改訂）**: `PostComposer` に可視性選択（👥パブリック/🤫ひかえめ/🔒️プライベート）を追加。Bsky（AT Protocol）はプロトコル上 followers_only（プライベート）配信のみ非対応のため、Bsky配送ON中にプライベートへ変更しようとする、またはプライベート中にBsky配送をONにしようとすると、フロントは軽量な自作ポップオーバー（`.popover`、汎用Tooltipコンポーネントは無いためこの機能に閉じて実装）でガイドし変更をブロックする（ひかえめはBsky配送と両立可能）。投稿APIはMisskey互換のためフロントを経由しない外部クライアントにはエラーを返せず、サーバー側（`create_regular_post`）でプライベート + Bsky配送要求を検知すると黙ってFediのみ配送に読み替える（`delivery.rs` の `deliver_regular_post`・`deliver_repost` でも二重に防御）。
* **可視性による閲覧制御（2026-07-17）**: `followers_only`/`direct` は投稿者本人または accepted フォロワーのみ閲覧可、`unlisted` はローカル（グローバル）タイムラインからのみ除外する（ホームタイムラインには表示される、詳細な判定ロジック・SQLガード式はDoc1 §1.3「可視性による閲覧制御」参照）。HTTP公開エンドポイントは可視性チェック付きの `PostRepository::find_by_id_for_viewer` を使い、ジョブ処理等の内部整合性チェックのみ従来の `find_by_id`（チェック無し）を使う。
* **リポスト・リプライの可視性継承（2026-07-17）**: プライベート（`followers_only`）ポストはリポスト不可（403エラー、`NoteCard` の🔁ボタンも無効化表示）。ひかえめ（`unlisted`）ポストのリポストは可能で、リポスト自体もひかえめを継承し、AP `Announce` の `to`/`cc` にも反映される。リプライは親ポストの可視性に応じて制約される（親がプライベート→リプライも強制的にプライベートかつBsky配送不可、親がひかえめ→public/unlisted/followers_only いずれも選択可でデフォルトひかえめ）。フロント `PostComposer` は返信時にも親の可視性に応じた選択肢（または強制表示）を出す（Doc1 §1.3「リポスト・リプライの可視性継承」参照）。
...
### 2.12 プロフィールのキーバリュー項目（#62）

Mastodon 等の「プロフィールのメタデータ欄」（AP Actor の `attachment[type=PropertyValue]`）
に相当する、ラベル+値のペアを最大4件まで設定できる機能（`actors.profile_fields`、Doc1 §1.2）。
ローカルユーザーが設定した項目は Fedi 向けプロフィール（Actor の `attachment`）に反映する。
Bsky には構造化フィールドが無いため、`description`（自己紹介文）末尾に `ラベル: 値` の
リストとして追記する（マイケルの提案通り）。逆方向として、リモート Fedi アクターが設定して
いる同種の項目も取り込んで表示する（Bsky 側の受信は対象外、送信のみ）。詳細は Doc3 §11.4。

* **画面配置**: プロフィール画面の bio 表示直下、DID/URI の「プロトコルアイデンティティ」
  ブロックの直前に、同じ `styles.identity`/`styles.idRow` を再利用したラベル+値のリストを
  表示する。値が `http(s)://` で始まる場合はリンクにする。
* **編集**: `/settings/profile` に固定4行のラベル+値入力フォームを追加（Mastodon のデフォルト
  UI に合わせた設計）。空行は保存時に除外する。

### 2.13 リスト機能（#63）

ユーザーごとに複数のリストを持て、ローカル/リモートFedi/Bskyのアクターを混在して
メンバー登録できる機能（`lists`/`list_members`テーブル、Doc1 §1.14）。公開リストは
Fediverse には AP Collection として、Bluesky には `app.bsky.graph.list`/`listitem`
として実際にPDSリポジトリへコミットし、双方のプロトコルからネイティブに閲覧できる
（Mastodon本家はリストを常に非公開にしか対応しないが、seiranはこの点を独自拡張して
いる）。プロトコル間の可視性制限（Fediからはbskyメンバーが見えない、逆も同様）は
それぞれの公開ハンドラ側で `actor_type` によるフィルタとして実装する（詳細は Doc3 §14）。

誰にもフォローされていないリモートFediユーザーの投稿を受信するため、seiranは
`list-relay` という仮想アクター（プロキシ）を持ち、リストに追加されたFediユーザーを
代理でリモートフォローする。フォロー要否は**参照カウント方式**（そのFediユーザーが
どのリストからも参照されなくなったら自動アンフォロー）で管理する。プロキシアクターは
`actor_type='local'`・`user_id=NULL`の`actors`行として表現し、AP署名はサーバー単一鍵を
流用するため専用の鍵ペア生成は不要（`seiran_common::system_actor`）。Bsky側の投稿受信は
Jetstreamの保存可否判定に「いずれかのリストに含まれるDID」を追加するだけで完結し、
プロキシフォロー相当の仕組みは不要（Bsky側はDB側フィルタのみで実現）。

* **画面配置**: ホーム画面のタイムラインペインに、リストごとのタブが追加される
  （`HomePage.tsx`、タブ数が多い場合は横スクロール）。自分のリスト管理は
  `/settings/lists`（`ListsSettingsPage.tsx`、左ペインナビゲーション`LeftNav.tsx`にも
  直接リンクを配置）、他人の公開リストは相手のプロフィール画面から一覧表示
  （`ProfilePage.tsx`の`listsSection`）し、`/lists/:id`（`ListDetailPage.tsx`）で
  リストタイムライン＋メンバー一覧を閲覧できる。`ListsSettingsPage.tsx`は3ペイン構成に
  沿い、中央ペインに自分のリスト一覧＋新規作成フォーム、右ペインに選択中リストの
  編集・メンバー管理を表示する（右ペインが無い狭幅環境では`ProfilePage.tsx`と同じ
  `matchMedia`判定で中央ペインに連続表示するフォールバックを持つ）。
* **メンバー追加のサジェスト**: メンバー追加入力欄は、DB上に既に存在するアクターを
  ユーザー名・表示名の部分一致（300msデバウンス）でサジェスト表示する
  （`GET /api/actors/search`、`handlers::actor_search`）。`username`/`domain`が別カラムの
  ため、`@user@domain`形式の入力にも対応できるよう、先頭`@`除去＋
  `username || '@' || domain`結合列も検索対象に含める。list-relay等の`users`行を持たない
  システムアクターは候補から除外する。
* **上限**: 1アクターあたりリスト数30個、1リストあたりメンバー数500人（提案値）。
* **既知の制約（MVP）**: リモートFedi/Bskyユーザー自身が持つ公開リストをオンデマンド
  取得・表示する機能は未実装（`ProfileResponse.public_lists`はローカルユーザーのみ対応）。
  また、公開済みリストの名前変更はATP側レコードの内容に追従しない（非公開→公開の
  再トグルで再コミットされるまでの間、ATP側の表示名が古いまま残る）。
