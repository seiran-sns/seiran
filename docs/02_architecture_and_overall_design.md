...
* **配送先・可視性バッジ（2026-07-16）**: `NoteCard` のヘッダ（アイコン名の `.acct` 行、既存の `protocolBadge` バッジの隣）に、ローカル投稿の配送先（🌐=Fedi配送あり、🦋=Bsky配送あり。`lib/format.ts` の `deliveryBadges`）と、Fedi受信ポストの可視性（🔒=フォロワーのみ、🏠=unlisted。同 `visibilityBadge`）を表示する。いずれも既存の `.protoBadge` CSSクラスを流用した表示専用バッジで、タイムラインのフィルタリングには関与しない。配送先はDB永続化した `posts.deliver_fedi`/`deliver_bsky`（投稿作成時の選択をそのまま保存）、可視性は AP受信時に `to`/`cc` から判定した `posts.visibility` に基づく（Doc1 §1.3「配送先・可視性アイコン表示」参照）。ローカル投稿の可視性選択は未実装のため常に `public`（バッジ無し）。
