...
既知の制約: 退会時にAP/ATP双方でフォロイー（自分がフォローしていた相手）側へ関係解消を
通知する処理は未実装（Doc6既知の課題参照、`wantedDids`除外は済んでいるがリモート側の
フォロー関係自体は解消されない）。Likeの通知insert（後述）に一意制約が無く、複線受信時に
重複する既知のバグも残っている（Doc6既知の課題参照）。
