...
既知の制約: `firehose`ロールを複数インスタンス起動した場合の排他制御は未実装（Doc6
既知の課題参照）。また、退会時にAP/ATP双方でフォロイー（自分がフォローしていた相手）側へ
関係解消を通知する処理も未実装（Doc6既知の課題参照、`wantedDids`除外は済んでいるが
リモート側のフォロー関係自体は解消されない）。
