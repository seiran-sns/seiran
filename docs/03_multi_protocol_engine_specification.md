...
### 1.1 AT Protocol (Bluesky) 同期シーケンス
1. フォローAPIの正常完了後、`actor_history_sync` キューに対象アクターの `at_did` をジョブ引数としてエンキューする。
2. ワーカーは、AppViewの `app.bsky.feed.getAuthorFeed` を呼び出す。
...
| **5. ATPリポジトリコミット・配信キュー**<br>`atp_repository_publish` | AT Protocol（ATP）のリポジトリ更新（CreateRecord / DeleteRecord）のローカルコミットと、リレーサーバー（Relay / Firehose）への通知・同期。 | **極高 (Critical)** | ・順序性の維持が不可欠（同じDIDに対するコミット順が前後してはならない）。<br>・アクターID単位の **FIFO（先入れ先出し）** キュー、またはシリアル排他ロック制御が必要。 |
