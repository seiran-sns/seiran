...
### DB マイグレーション
- [ ] `user_role` ENUM 型（`user` / `moderator` / `admin`）を追加し `users.role` カラムを追加
- [ ] `users.suspended_at TIMESTAMPTZ` カラムを追加
- [ ] `actors.avatar_media_id` / `actors.banner_media_id` カラムを追加
- [ ] `storage_providers` テーブル新規作成（スキーマ: `docs/01_database_schema_blueprint.md` 1.7）
- [ ] `media_files` テーブル新規作成（スキーマ: 1.8）
- [ ] `post_attachments` テーブル新規作成（スキーマ: 1.9）
- [ ] `custom_emojis` テーブル新規作成（スキーマ: 1.10）

### secrets.toml
- [ ] `encryption_key`（256bit、AES-256-GCM 用）を自動生成・永続化

### オブジェクトストレージ統合
- [ ] `storage_providers` CRUD（管理者 API）
  - `secret_key` は `encryption_key` で AES-256-GCM 暗号化して格納
- [ ] ストレージ選択アルゴリズム実装（id 順・`capacity_mb` チェック）
- [ ] S3 互換 PUT / DELETE クライアント実装（`aws-sdk-s3` または `object_store` クレート）

### 画像処理パイプライン
- [ ] WebP 変換・リサイズ処理実装（`image` クレート）
  - アバター: 中央正方形クロップ → 600 × 600
  - バナー: fit inside → 横最大 2048 × 縦最大 768
  - カスタム絵文字: fit inside → 横最大 384 × 縦最大 64
  - ポスト添付: fit inside → 長辺最大 2048
  - アニメーション WebP は animated モードで処理
- [ ] SHA-256 + blurhash 計算（WebP 変換後に算出）
- [ ] `(sha256, blurhash)` 複合一致による重複排除
- [ ] クォータチェック実装（ポスト送信・プロフィール保存時に実参照から計算）

### アップロード API
- [ ] `POST /api/drive/files/create` — 画像アップロード（Misskey 互換）
...
### 管理画面（バックエンド API）
- [ ] `role` チェックミドルウェア実装（`admin` / `moderator` でエンドポイントを保護）
- [ ] `GET /api/admin/storage-providers` / `POST` / `PATCH` / `DELETE`
- [ ] `GET /api/admin/users` — ユーザー一覧（凍結状態含む）
- [ ] `POST /api/admin/users/:id/suspend` / `unsuspend`
- [ ] `POST /api/admin/users/:id/role` — ロール変更（admin のみ）
- [ ] `GET/POST/DELETE /api/admin/emojis` — カスタム絵文字管理
