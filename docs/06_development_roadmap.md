...
- [ ] **4.5.1. ローカル機能 API の実装 (seiran-api)**
  - [ ] `POST /api/auth/register` — ユーザー登録（Argon2ハッシュ、users + actors テーブル保存）
  - [ ] `POST /api/auth/login` — ログイン（パスワード検証、JWT発行）
  - [ ] `GET /api/auth/me` — 自分のユーザー情報取得（JWT認証）
  - [ ] `POST /api/notes/create` — ローカル投稿作成
  - [ ] `GET /api/notes/local-timeline` — ローカルタイムライン（`sinceId` / `untilId` ページネーション）
- [ ] **4.5.2. フロントエンド (React + Vite + TypeScript)**
  - [ ] `frontend/` プロジェクト初期化
  - [ ] ログイン画面 ・ ユーザー登録画面
  - [ ] ローカルタイムライン画面
  - [ ] 投稿入力フォーム
