...
- [x] **4.2. AT Protocol (Bluesky) 統合**
  ...
  - [x] seiran PDS としてのローカルユーザー ATP リポジトリ管理（MST署名コミット + Relay 配信）
    ...
    - [x] **feat: 音声投稿をグレー背景動画に変換し`app.bsky.embed.video`として配信**（2026-07-17、マイケル発案・実機検証。Doc3 §12.6）: `convert_audio_to_gray_video`が`ffmpeg`で音声を80x20(4:1)の無音映像と結合したmp4に変換し、動画パイプラインへ提出する。515x75は"aspect ratio too wide"、36x12は"video processing error"で拒否され、80x20で実機動作確認。副次的に`media_files.bsky_video_size`（トランスコード後の実バイト数、通常の動画embedにも影響する既存の不正確さだった）と、`uploadVideo`失敗時のエラー詳細が空jobIdのせいで握り潰されていたバグも修正。
