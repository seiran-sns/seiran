...
- [x] **7.3. Webフロントエンド（React + Vite）の3ペインUIの実装**
  ...
  - [ ] TODO: Likeの通知insert（`firehose.rs`の`handle_inbound_like_create`、`notifications`テーブル）に一意制約が無く、複線受信で通知が重複して2行DBに残るバグが残っている（上記の排他制御でJetstream接続自体の重複は防げるが、無停止バージョンアップ中の一時的な複線接続では依然発生しうる）。ATP Likeレコードの`at_uri`（`app.bsky.feed.like`のrkey由来、1いいね=1レコードで一意）を`notifications`に持たせ部分ユニークインデックス（`source_uri`列、`WHERE source_uri IS NOT NULL`）で防げる設計案あり。AP側Reaction通知（`inbound_activity_process.rs`、`ap_activity_id`）も同型のリスクを抱えており、同じ仕組みで一緒に塞げる（2026-07-16 マイケル承認、スコープ外として保留）。
...
- [x] **7.3.x. 退会機能 Phase A（#29）**
  ...
  - [ ] TODO: 退会時、自分がフォローしていた相手（フォロイー）側への関係解消通知が未実装。`deliver_delete_actor`（Doc3参照）は`fetch_fedi_follower_inboxes`で**自分のFediフォロワー**にのみDelete(Actor)を配送しており、フォロイー側のリモートサーバーには何も届かない。ATP側も`broadcast_account_event`（#accountイベント）のみで`app.bsky.graph.follow`の個別解除は送らない。`follows`/`list_members`のレコード自体も退会後に物理削除されず残存する（2026-07-16 マイケル指摘、スコープ外として保留）
