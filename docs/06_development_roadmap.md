...
- [ ] **3.1. `JobQueue` Trait (インターフェース) の定義**
  - [ ] ジョブのプッシュ、ポップ、リトライ、キャンセル等のメソッド定義
- [ ] **3.2. キュー実装の切り替え対応**
  - [ ] `InMemoryJobQueue` の実装 (開発初期用、オンメモリ)
  - [ ] `RedisJobQueue` の実装 (本番スケール用、`apalis` 等を活用したRedis連携)
- [ ] **3.3. 5大非同期ジョブハンドラの実装**
  - [ ] **① 過去ログ同期キュー (`actor_history_sync`)**
    - [ ] ドメイン単位 of 同時実行制限（Concurrency Limit）の適用
    - [ ] 1〜3秒のジッター挿入、指数バックオフで最大3回リトライ
  - [ ] **② 投稿配送キュー (`outbound_post_delivery`)**
    - [ ] 高優先度処理、相手サーバーダウン時の長期指数バックオフ（最大10回リトライ）
  ...
  - [ ] **⑤ ATPリポジトリコミット・配信キュー (`atp_repository_publish`)**
    - [ ] 極高優先度、アクターID単位のFIFO（先入れ先出し）制御・排他ロックの適用
...
- [ ] **4.2. AT Protocol (Bluesky) 統合**
  ...
  - [ ] PDS（Personal Data Server）へのコミット、Firehose同期モジュールの実装
...
## 🎨 フェーズ7: フロントエンド（3ペインUI） ＆ Misskey API互換レイヤー (UI & API)
Misskey API互換レイヤーを完成させ、公式Webフロントエンド（Vue.js/Viteフォーク）の3ペインUIと連携させる。

- [ ] **7.1. Misskey API互換エンドポイントの実装 (バックエンド)**
  - [ ] `/api/notes/timeline`, `/api/notes/create`, `/api/notes/search` 等の主要エンドポイントの実装
...
- [ ] **7.3. 公式Webフロントエンド（Vue.js/Vite）の3ペインUIの実装**
...
- [ ] **7.4. サードパーティ製クライアント向けのフォールバック処理**
