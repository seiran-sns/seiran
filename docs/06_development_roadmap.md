...
- [x] **7.3. Webフロントエンド（React + Vite）の3ペインUIの実装**
  ...
  - [ ] TODO: `firehose`ロール（Jetstream接続、`seiran-atp-repo`）を複数インスタンス起動した場合の排他制御が未実装。`docker-compose.mono.yml`は`--scale seiran-server=N`によるスケールアウトを前提としているが、`all`ロールにfirehose機能が同梱されているため、そのままスケールするとJetstream WebSocket接続がスケール数だけ重複して張られる。投稿保存は`ON CONFLICT DO NOTHING`で冪等なためデータ破損はしないが、接続本数・DB問い合わせが無駄に増える。リーダー選出（`pg_advisory_lock`等）の導入が必要（2026-07-16 マイケル指摘、スコープ外として保留）
