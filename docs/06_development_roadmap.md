...
- [x] **4.2. AT Protocol (Bluesky) 統合**
  ...
  - [ ] **リポスト重複制約**: `UNIQUE INDEX (actor_id, repost_of_post_id) WHERE deleted_at IS NULL` をマイグレーションで追加（取り消し前の再リポストを DB レベルで禁止）
  - [ ] **リポストのクロスプロトコル配信**（spec doc 9.1〜9.3 節参照）
    - [ ] Fedi リモートポストのリポスト → Bsky: DB の at_uri 確認 → ブリッジ探索（タイムアウト 2 秒）→ フォールバック URL カード投稿
    - [ ] Bsky リモートポストのリポスト → Fedi: DB の ap_object_id 確認 → ブリッジ探索（タイムアウト 2 秒）→ フォールバック URL テキスト投稿
    - [ ] ローカル / リモート seiran ポストのリポスト → 両方に配信
  - [ ] **引用投稿のクロスプロトコル配信**（spec doc 9.4 節参照）
    - [ ] Fedi 引用 → Bsky: `app.bsky.embed.external`（元ポスト URL カード）で代替
    - [ ] Bsky 引用 → Fedi: quoteUrl として bsky.app URL を付与
  - [ ] **リプライの配信先制御**（spec doc 9.5 節参照）
    - [ ] 元ポストが Fedi リモートならリプライを Fedi のみに配信（Bsky 配信しない）
    - [ ] 元ポストが Bsky リモートならリプライを Bsky のみに配信（Fedi 配信しない）
    - [ ] ローカル / リモート seiran ポストへのリプライは両方に配信
...
- [ ] WebP 変換後サイズが元画像より大きい場合は元形式（gif/jpeg/png/avif）のまま保存する
  - `media_files.sha256` は「実際に保存したバイナリ」のハッシュ値を持つ（WebP なら WebP のハッシュ、元画像なら元画像のハッシュ）
  - 重複チェックの二段構え: ① 元画像のハッシュ・blurhash で既存と比較 → ② WebP 変換後のハッシュ・blurhash でも比較
  - `media_files.mime_type` は保存形式に合わせて更新（WebP でなければ元の MIME type を保持）
  - 元形式保存時も Exif の位置情報（GPS タグ）は必ず除去する

### アップロード API
- [x] `POST /api/drive/files/create` — 画像アップロード（Misskey 互換）
  - オブジェクトストレージ未設定時は `503` を返す
  - ドライブ UI はフロントエンドに公開しない
- [x] ポスト添付: `post_attachments` に保存（上限 4 枚、AT Protocol 互換）
- [ ] ポスト添付上限を 10 枚に拡張（Bsky が 2026-06 に 10 枚対応済み。UI・バリデーション・Bsky embed 両方変更必要）
