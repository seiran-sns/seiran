...
- [x] **4.5.1. ローカル機能 API の実装 (seiran-api)**
  ...
  - [x] `GET /notes/:id` — ActivityPub Note エンドポイント（Fediverse 向け、ローカルポストのみ）
    - ブラウザ向け詳細画面は `/post/:id` にパスを分離済み。nginx はコンテンツネゴシエーションなしで
      `/notes/:id` を常に api へ、`/post/:id` を frontend へ静的に転送する
...
- [x] **4.5.2. フロントエンド (React + Vite + TypeScript)**
  ...
  - [x] ポスト詳細画面（`/post/:id`）: 単一投稿表示。AP エントリーポイントは `/notes/:id`（api 側）に分離
